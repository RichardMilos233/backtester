...
from src.signals import compute_moving_average_signal, compute_regime_filtered_signal, compute_volatility_scaled_signal
...
base_signal = compute_moving_average_signal(df_reg, window=50)
scaled_signal = compute_volatility_scaled_signal(df_reg, base_signal)

# 3. 运行真实回测（产出带 regime 的账本 res）
bt = Backtest(initial_capital=100000.0)
res = bt.run(df_reg, scaled_signal)
